#!/usr/bin/env python3
"""Install JoySword 7-Day Cumulative Login Streak Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-login-streak.sql"
...
cat > /tmp/joysword-login-streak.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-login-streak.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing login streak installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"Login streak install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("Login streak DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "7-Day Login Streak procedure installed.").strip())
    return 0
