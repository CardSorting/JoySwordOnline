#!/usr/bin/env python3
"""Install JoySword First-Time Top-Up 2x Cash Bonus Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-first-topup-bonus.sql"
...
cat > /tmp/joysword-first-topup.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-first-topup.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing first-topup installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"First-TopUp install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("First-TopUp DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "First-TopUp 2x Bonus procedure installed.").strip())
    return 0
