#!/usr/bin/env python3
"""Install JoySword 50-Tier BattlePass Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-battlepass-system.sql"
...
cat > /tmp/joysword-battlepass.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-battlepass.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing battlepass installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"BattlePass install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("BattlePass DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "BattlePass procedure installed.").strip())
    return 0
