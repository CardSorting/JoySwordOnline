...
foreach ($Db in $Databases) {
    ...
    $FileList = & $SqlCmd -S "localhost\SQLEXPRESS" -U sa -P $DBPassword -C -h -1 -W -Q $FileListQuery
...
"@
    ...
    & $SqlCmd -S "localhost\SQLEXPRESS" -U sa -P $DBPassword -C -b -Q $RestoreQuery
...
& $SqlCmd -S "localhost\SQLEXPRESS" -U sa -P $DBPassword -C -b -v ADMIN_USER="$($EnvVars['ADMIN_USER'])" ADMIN_PASSWORD="$($EnvVars['ADMIN_PASSWORD'])" ADMIN_STARTING_CASH="$($EnvVars['ADMIN_STARTING_CASH'])" -i $SeedFile
if ($LASTEXITCODE -ne 0) {
...
GO
"@
& $SqlCmd -S "localhost\SQLEXPRESS" -U sa -P $DBPassword -C -b -Q $AlterQuery
if ($LASTEXITCODE -ne 0) {
