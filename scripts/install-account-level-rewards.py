#!/usr/bin/env python3
"""Install JoySword Account Level Milestone Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-account-level-rewards.sql"
...
cat > /tmp/joysword-account-level.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-account-level.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing account level installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"Account level install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("Account level DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "Account Level procedure installed.").strip())
    return 0
