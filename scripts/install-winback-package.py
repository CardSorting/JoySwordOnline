#!/usr/bin/env python3
"""Install JoySword Winback Welcome Package Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-winback-package.sql"
...
cat > /tmp/joysword-winback-package.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-winback-package.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing winback installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"Winback package install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("Winback package DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "Winback Package procedure installed.").strip())
    return 0
