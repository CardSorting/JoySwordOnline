#!/usr/bin/env python3
"""Install JoySword Daily ED Allowance Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-ed-allowance.sql"
...
cat > /tmp/joysword-ed-allowance.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-ed-allowance.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing ED allowance installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"ED allowance install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("ED allowance DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "Daily ED allowance procedure installed.").strip())
    return 0
