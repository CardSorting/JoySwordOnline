#!/usr/bin/env python3
"""Install JoySword Monthly Paragon Pass Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-monthly-pass-system.sql"
...
cat > /tmp/joysword-monthly-pass.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-monthly-pass.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing monthly pass installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"Monthly pass install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("Monthly pass DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "Monthly Pass procedure installed.").strip())
    return 0
