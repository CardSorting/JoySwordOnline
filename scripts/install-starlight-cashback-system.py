#!/usr/bin/env python3
"""Install JoySword Starlight Cashback Token Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-starlight-cashback-system.sql"
...
cat > /tmp/joysword-starlight.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-starlight.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing starlight installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"Starlight Cashback install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("Starlight Cashback DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "Starlight Cashback procedure installed.").strip())
    return 0
