#!/usr/bin/env python3
"""Install JoySword 10-Tier VIP Loyalty Policy & Stored Procedure."""
...
SQL_FILE = ROOT / "database" / "install-vip-tier-system.sql"
...
cat > /tmp/joysword-vip-tier.sql
"$CMD" -S localhost -U sa -P "$MSSQL_SA_PASSWORD" "${EXTRA[@]}" -b -i /tmp/joysword-vip-tier.sql
"""
...
def main() -> int:
    if not SQL_FILE.exists():
        print(f"Missing VIP installer: {SQL_FILE}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"VIP tier install failed: {exc}", file=sys.stderr)
        return 1
    ...
    if result.returncode != 0:
        print("VIP tier DB install notice:", file=sys.stderr)
        if result.stdout:
    ...
    print((result.stdout or "VIP Tier procedure installed.").strip())
    return 0
