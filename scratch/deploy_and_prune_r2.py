...
print("\nScanning R2 bucket for old releases to prune...")
try:
    paginator = s3.get_paginator('list_objects_v2')
    pages = paginator.paginate(Bucket=bucket_name, Prefix="releases/")
    ...
    for page in pages:
        if 'Contents' in page:
            for obj in page['Contents']:
                key = obj['Key']
                # Check if it does NOT belong to the active version 1.6.0
                if not key.startswith(f"releases/{version}/"):
                    keys_to_delete.append({'Key': key})
                    
    if keys_to_delete:
        print(f"Found {len(keys_to_delete)} old objects to delete:")
        ...
        print(f"Successfully deleted {len(keys_to_delete)} old objects from R2!")
    else:
        print("No old release objects found to prune.")
except Exception as e:
    print(f"Error pruning old R2 objects: {e}", file=sys.stderr)
    sys.exit(1)

print("\nR2 Deployment and Cleanup finished successfully!")
