...
----------     Center NetLayer settings     ----------

NetLayer:SetCheckSequenceNum( false ) -- 내부통신은 SequenceNum 체크필요없음 dont touch this please - psn


----------     Center SimLayer Settings     ----------
...
--else
--	DBLayer:AddDB( DC_GAME,		'game01_GAIA.dsn',50, false )
--	DBLayer:AddDB( DC_LOG,		'log_US_SERVICE.dsn',		30, false )
--end

--DBLayer:AddDB( DC_SMS,		'SMS.dsn',			1, false )

----------     Room settings for ingame(i.e pvp/dungeon/raid etc)     ----------

RoomManager:SetNumPvPRoom( 1500 )
RoomManager:SetNumDungeonRoom( 1500 )
RoomManager:SetNumSquare( 10 ) -- DB에 설정되어 있는 광장 개수 이상이어야 한다. Has to be above what is set on the DB(im not entirely sure what this is)
...
-- 좀비유저 체크 시간 설정 // Zombie user check settings --
...
-- log level settings(self explantory lol) --
--[[
-1 : no logs
 0 : cerr, cout
 1 : cerr, cwarn, cout
