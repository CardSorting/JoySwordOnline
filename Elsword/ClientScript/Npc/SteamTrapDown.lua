...
INIT_STATE = 
{
	{ STATE_NAME = "STEAM_TRAP_DOWN_WAIT",						LUA_STATE_START_FUNC = "STEAM_TRAP_DOWN_WAIT_STATE_START", },
	{ STATE_NAME = "STEAM_TRAP_DOWN_ATTACK",					LUA_FRAME_MOVE_FUNC = "STEAM_TRAP_DOWN_ATTACK_FRAME_MOVE"	},

	START_STATE					= "STEAM_TRAP_DOWN_WAIT",
	...
	DOWN_DAMAGE_AIR				= "",
	UP_DAMAGE					= "",
	DAMAGE_REVENGE				= "STEAM_TRAP_DOWN_WAIT",
	
	DYING_LAND_FRONT			= "",
	DYING_LAND_BACK				= "",
	DYING_SKY					= "",

	REVENGE_ATTACK				= "STEAM_TRAP_DOWN_WAIT",	
}
...
STEAM_TRAP_DOWN_WAIT = 
{
	...
	EVENT_PROCESS = 
	{		
		{ STATE_CHANGE_TYPE["SCT_CONDITION_TABLE"],			"STEAM_TRAP_DOWN_ATTACK",				"CT_STEAM_TRAP_DOWN_ATTACK",	},
	},
	
	CT_STEAM_TRAP_DOWN_ATTACK = 
	{
		FLAG_FALSE = 0,
	},
}

STEAM_TRAP_DOWN_ATTACK = 
{
	ANIM_NAME					= "Attack",
	PLAY_TYPE					= XSKIN_ANIM_PLAYTYPE["XAP_LOOP"],
	TRANSITION					= FALSE,
	...
	EVENT_PROCESS = 
	{		
		{ STATE_CHANGE_TYPE["SCT_CONDITION_TABLE"],				"STEAM_TRAP_DOWN_WAIT",		"CT_STEAM_TRAP_DOWN_WAIT",			},
	},

	
	CT_STEAM_TRAP_DOWN_WAIT = 
	{
		FLAG_TRUE = 0,
	},
}
...
function STEAM_TRAP_DOWN_ATTACK_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )
                ...
                if pMajorParticle ~= nil then
                    ...
                    if pUnit:GetNowHP() > pUnit:GetMaxHP() * 0.3 then
	                    ...
	                    if pParticle == nil then
	                        particle = pMajorParticle:GameUnitCreateSequence_LUA( pNPCUnit, "SteamAttackDown", vpos, D3DXVECTOR2(100,100), D3DXVECTOR2(-1,-1) )    	
                        ...
                        if pUser ~= nil then
                            ...
                            unitPos = pNPCUnit:GetPos()
                            
                            fDist = GetDistance_LUA( userPos, unitPos )
                            if fDist <= 1500 then
                                -- 푸슈~~~ 루핑 (캐릭터와 스팀과의 거리가 1500 이하이면 소리남)
                                if pNPCUnit:AnimEventTimer_LUA( 0.01 ) then
                                    pNPCUnit:PlaySound_LUA( "SteamTrap_Fire.ogg" )                                   
                                    pNPCUnit:ClearEventCheck(0.01)
                                end                                    
                                if pNPCUnit:AnimEventTimer_LUA( 2.97 ) then	    
                                    pNPCUnit:PlaySound_LUA( "SteamTrap_Fire.ogg" )                                    
                                    pNPCUnit:ClearEventCheck(2.97)
                                end  
                            end
                        end                    
...
function STEAM_TRAP_DOWN_WAIT_STATE_START( pKTDXApp, pX2Game, pNPCUnit )
