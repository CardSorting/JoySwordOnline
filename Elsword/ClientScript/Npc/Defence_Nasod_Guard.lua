...
INIT_MOTION = 
{
	...
	MOTION_CHANGE_TEX_XET		= "DEFENCE_NASOD_GUARD.xet",	
	MOTION_ANI_TEX_XET			= "DEFENCE_NASOD_GUARD.xet",
}
...
INIT_STATE = 
{
	...
	{ STATE_NAME = "NASOD_GUARD_WAIT",							LUA_FRAME_MOVE_FUNC = "NASOD_GUARD_WAIT_FRAME_MOVE", },
	{ STATE_NAME = "NASOD_GUARD_WAIT_START",					},
	
	{ STATE_NAME = "NASOD_GUARD_WALK",							LUA_FRAME_MOVE_FUNC = "NASOD_GUARD_WALK_FRAME_MOVE",
																LUA_STATE_END_FUNC = "NASOD_GUARD_WALK_STATE_END"						},
...
INIT_AI = 
{
	TARGET = 
	{
		...
		TARGET_NPC					= TRUE,
		MANUAL_TARGETING_FUNC		= "NASOD_GUARD_MANUAL_TARGET",
	},
...
--타겟 관련 펑션
function NASOD_GUARD_MANUAL_TARGET( pKTDXApp, pX2Game, pNPCUnit )
...
function NASOD_GUARD_WALK_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )
	...
	if pX2Game:AnyUserUnitInRange( pNPCUnit:GetPos(), 200 ) == true then
		...
		if pNPCUnit:IsRightTarget_LUA(pNPCUnit, TargetPos) == true then
			if( bIsRight == true ) then
				pNPCUnit:StateChange_LUA( "NASOD_GUARD_COMBO" )
			end
		else
...
function NASOD_GUARD_WAIT_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )
	...
	if pX2Game:AnyUserUnitInRange( pNPCUnit:GetPos(), 200 ) == true then
		...
		if pNPCUnit:IsRightTarget_LUA(pNPCUnit, TargetPos) == true then
			if( bIsRight == true ) then
				pNPCUnit:StateChange_LUA( "NASOD_GUARD_ATTACK" )
			end
		else
			if( bIsRight == false ) then
				pNPCUnit:StateChange_LUA( "NASOD_GUARD_ATTACK" )
