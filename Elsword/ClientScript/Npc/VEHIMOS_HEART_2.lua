...
INIT_STATE = 
{
	{ STATE_NAME = "VEHIMOS_HEART_2_START",							LUA_STATE_END_FUNC = "VEHIMOS_HEART_2_START_END",				},
	{ STATE_NAME = "VEHIMOS_HEART_2_WAIT",							},
	
	--Red Egg를 소환
	{ STATE_NAME = "VEHIMOS_HEART_2_SUMMON_A",						LUA_FRAME_MOVE_FUNC = "VEHIMOS_HEART_2_SUMMON_A_FRAME_MOVE",
																	LUA_STATE_END_FUNC = "VEHIMOS_HEART_2_SUMMON_A_END",			},
	--생체, 변종 촉수를 소환
	{ STATE_NAME = "VEHIMOS_HEART_2_SUMMON_B",						LUA_FRAME_MOVE_FUNC = "VEHIMOS_HEART_2_SUMMON_B_FRAME_MOVE",	
																	LUA_STATE_END_FUNC = "VEHIMOS_HEART_2_SUMMON_B_END",			},
	
	{ STATE_NAME = "VEHIMOS_HEART_2_DYING",							},
	{ STATE_NAME = "VEHIMOS_HEART_2_DAMAGE",							},

	START_STATE					= "VEHIMOS_HEART_2_START",
	WAIT_STATE					= "VEHIMOS_HEART_2_WAIT",
	
	SMALL_DAMAGE_LAND_FRONT		= "VEHIMOS_HEART_2_DAMAGE",
	SMALL_DAMAGE_LAND_BACK		= "VEHIMOS_HEART_2_DAMAGE",
	BIG_DAMAGE_LAND_FRONT		= "VEHIMOS_HEART_2_DAMAGE",
	BIG_DAMAGE_LAND_BACK		= "VEHIMOS_HEART_2_DAMAGE",
	DOWN_DAMAGE_LAND_FRONT		= "VEHIMOS_HEART_2_DAMAGE",
	DOWN_DAMAGE_LAND_BACK		= "VEHIMOS_HEART_2_DAMAGE",
	
	DYING_LAND_FRONT			= "VEHIMOS_HEART_2_DYING",
	DYING_LAND_BACK				= "VEHIMOS_HEART_2_DYING",
	DYING_SKY					= "VEHIMOS_HEART_2_DYING",
...
VEHIMOS_HEART_2_START = 
{
	...
	EVENT_PROCESS = 
	{
		{ STATE_CHANGE_TYPE["SCT_MOTION_END"],		"VEHIMOS_HEART_2_WAIT",							},
		
	},
}
function VEHIMOS_HEART_2_START_END( pKTDXApp, pX2Game, pNPCUnit )
...
VEHIMOS_HEART_2_WAIT = 
{
	...
	EVENT_PROCESS =
	{
		{ STATE_CHANGE_TYPE["SCT_CONDITION_FUNCTION"],		"VEHIMOS_HEART_2_SUMMON_A",	"CF_VEHIMOS_HEART_2_SUMMON_A",	},
		{ STATE_CHANGE_TYPE["SCT_CONDITION_FUNCTION"],		"VEHIMOS_HEART_2_SUMMON_B",	"CF_VEHIMOS_HEART_2_SUMMON_B",	},
	},
...
function CF_VEHIMOS_HEART_2_SUMMON_A( pKTDXApp, pX2Game, pNPCUnit )
	...
	if pNPCUnit:GetInt_LUA( 0 ) < pNPCUnit:GetTimerElapsedTime( 0 ) then
		...
		if true == CheckAbleSummon( pX2Game, pNPCUnit ) then
			...
			--최대 소환 갯수보다 작으면 VEHIMOS_HEART_2_SUMMON_A state로 넘어감
...
function CF_VEHIMOS_HEART_2_SUMMON_B( pKTDXApp, pX2Game, pNPCUnit )
	...
	if pNPCUnit:GetInt_LUA( 1 ) < pNPCUnit:GetTimerElapsedTime( 1 ) then		
		...
		if true == CheckAbleSummon( pX2Game, pNPCUnit ) then
			...
			--최대 소환 갯수보다 작으면 VEHIMOS_HEART_2_SUMMON_B state로 넘어감
...
function VEHIMOS_HEART_2_SUMMON_A_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )

	if pNPCUnit:AnimEventTimer_LUA( 2.4 ) then
		if nil ~= pX2Game:GetLineMap() then
			local ArrSize = 14 --ArrPos 배열안의 인자 갯수와 숫자를 같게 해줘야 함.
			local ArrPos = { 87, 88, 89, 90, 91, 92, 93, 94, 95, 97, 98, 99, 101, 102,} --라인 맵의 스타트 포지션 지정 해줘야 함.
...
function VEHIMOS_HEART_2_SUMMON_A_END( pKTDXApp, pX2Game, pNPCUnit )
	--타이머 다시 리셋
	pNPCUnit:SetTimerRestart( 0 )
end

VEHIMOS_HEART_2_SUMMON_B = 
{
	ANIM_NAME					= "SummonB",
	PLAY_TYPE					= XSKIN_ANIM_PLAYTYPE["XAP_ONE_WAIT"],
	TRANSITION					= TRUE,
...
function VEHIMOS_HEART_2_SUMMON_B_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )

	if pNPCUnit:AnimEventTimer_LUA( 1.2 ) then
		if nil ~= pX2Game:GetLineMap() then
			local ArrSize = 9 --ArrPos 배열안의 인자 갯수와 숫자를 같게 해줘야 함.
			local ArrPos = { 87, 89, 90, 92, 93, 95, 98, 100, 102, } --라인 맵의 스타트 포지션 지정 해줘야 함.
...
function VEHIMOS_HEART_2_SUMMON_B_END( pKTDXApp, pX2Game, pNPCUnit )
	--타이머 다시 리셋
	pNPCUnit:SetTimerRestart( 1 )
end


VEHIMOS_HEART_2_DYING =
{
	ANIM_NAME					= "Dying",
	PLAY_TYPE					= XSKIN_ANIM_PLAYTYPE["XAP_ONE_WAIT"],
	TRANSITION					= FALSE,
