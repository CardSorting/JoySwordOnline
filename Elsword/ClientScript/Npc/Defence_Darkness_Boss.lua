...
DEFENCE_DARKNESS_BOSS_MAGICATTACK_B2 = 
{
	...
	SOUND_PLAY0					= { 0.837, "Dragon_RockAttack.ogg",100, FALSE, 999999, },
	SOUND_PLAY1					= { 0.837, "Dragon_RockAttack.ogg",100, FALSE, 999999, },
...
function AttackEffect_B1( pKTDXApp, pX2Game, pNPCUnit )					--B1 공격 이펙트
	...
	elseif pNPCUnit:AnimEventTimer_LUA( 1.12 ) then						--공격
	
		pNPCUnit:PlaySound_LUA( "Defence_Darkness_Boss_MagicAttack01.ogg" )
		pNPCUnit:PlaySound_LUA( "Defence_Darkness_Boss_MagicAttack01.ogg" )

		local vSummonPos = D3DXVECTOR3( 0, -721, 0 )						--1층
	
		local pEffectSet = pX2Game:GetEffectSet()

		pEffectSet:PlayEffectSetWithCustomPos_LUA( "EffectSet_darkness_attackB1_M01", pNPCUnit, vSummonPos, D3DXVECTOR3(0, 0, 0) )
...
function AttackEffect_B2( pKTDXApp, pX2Game, pNPCUnit )					--B2 공격 이펙트
	...
	elseif pNPCUnit:AnimEventTimer_LUA( 1.12 ) then						--공격
		...
		pEffectSet:PlayEffectSetWithCustomPos_LUA( "EffectSet_darkness_attackB2_M01", pNPCUnit, vSummonPos, D3DXVECTOR3(0, 0, 0) )
...
function AttackEffect_B3( pKTDXApp, pX2Game, pNPCUnit )					--B3 공격 이펙트
	...
	elseif pNPCUnit:AnimEventTimer_LUA( 1.12 ) then						--공격
		...
		pEffectSet:PlayEffectSetWithCustomPos_LUA( "EffectSet_darkness_attackB3_M01", pNPCUnit, vSummonPos, D3DXVECTOR3(0, 0, 0) )
...
function DEFENCE_DARKNESS_BOSS_COMMON_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )    
	...
	local pDarkBall = pX2Game:GetNPCUnitByType( NPC_UNIT_ID["NUI_DEFENCE_DARK_BALL_NORMAL"] )

	if pCrystal ~= nil then														--엘의 수정이 존재함
			
		if pCrystal:GetNowHP_LUA() > 0 then										--엘의 수정의 체력이 남아 있을 때
		
			if (iBlastBossKilled >= 7) then 									--Shodow Blast Boss 사망카운트 충족 or DarkBall의 소멸 --블라스트 보스 카운트 100이 되어야 사망 처리됨
				pNPCUnit:SetFlag_LUA(2, true)									--사망 인식 Boolean
				pNPCUnit:SetNowHP_LUA(0)										--사망	State
				return
			...
			if pDarkBall == nil then											--소환문이 소환을 마치고, 혼자 남을 때 사망 처리
				pNPCUnit:SetFlag_LUA(2, true)									--사망 인식 Boolean
	...
	local iSleepTimeHead2  = 150												--머리 2 깨는 시간
	local iSleepTimeHead3  = 300												--머리 3 깨는 시간
	...
	--상자 소환
	SUMMON_BOX( 27, 80, pX2Game, pNPCUnit)
...
function SUMMON_BOX(LineIndex1, fSummonInterval, pX2Game, pNPCUnit ) --상자 소환
