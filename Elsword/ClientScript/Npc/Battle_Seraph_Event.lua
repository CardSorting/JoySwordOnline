...
INIT_MOTION = 
{
	MOTION_FILE_NAME		= "Motion_EVENT_Battle_Seraph.x",
}
...
INIT_STATE = 
{	
	{ STATE_NAME = "APINK_ANGEL_01_ELSWORD_START",				LUA_STATE_START_FUNC = "APINK_ANGEL_01_ELSWORD_START_FRAME_MOVE"		},
	...
	-- 에너지 니들
	{ STATE_NAME = "APINK_ANGEL_01_ELSWORD_ATTACK_B",		STATE_COOL_TIME = 7,       	LUA_FRAME_MOVE_FUNC = "APINK_ANGEL_05_EVE_ATTACK_B_FRAME_MOVE" },


	-- 에너지틱 하트 모션으로 유도미사일 10발 발사!!
	{ STATE_NAME = "APINK_ANGEL_01_ELSWORD_ATTACK_D",		STATE_COOL_TIME = 15,        LUA_FRAME_MOVE_FUNC = "APINK_ANGEL_05_EVE_ATTACK_C_FRAME_MOVE" },
	...
	COMMON_FRAME_FUNC			= "APINK_ANGEL_01_ELSWORD_COMMON_FRAME_FUNC",
...
function APINK_ANGEL_01_ELSWORD_START_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )
	...
	pNPCUnit:SetVector_LUA(0, D3DXVECTOR3(120,	-40,  20))
	pNPCUnit:SetVector_LUA(1, D3DXVECTOR3(135,	-20, -20))
	pNPCUnit:SetVector_LUA(2, D3DXVECTOR3(105,	 60,   0))
	pNPCUnit:SetVector_LUA(3, D3DXVECTOR3(140,  75,   0))
	pNPCUnit:SetVector_LUA(4, D3DXVECTOR3(110,	-60,   0))
	pNPCUnit:SetVector_LUA(5, D3DXVECTOR3(100,	-75,  40))
...
function APINK_ANGEL_05_EVE_ATTACK_A_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )
	...
	if pNPCUnit:AnimEventTimer_LUA( 0.3 ) then		    
		...
		local pCEffect = pDamageEffect:CreateInstance_LUA( pNPCUnit, "EVE_TASER_PILUM_CHARGE_0", LHandPos, landPos.y )
...
function APINK_ANGEL_01_ELSWORD_COMMON_FRAME_FUNC( pKTDXApp, pX2Game, pNPCUnit )

	if pNPCUnit:GetFlag_LUA(0) == true then
		if pNPCUnit:GetTimerElapsedTime( 0 ) > 0.33 then		    
			...
			-- 랜덤 위치에서 발사되도록
			local RandomInt = RandomInt_LUA(0,5)
			local RandomVector = pNPCUnit:GetVector_LUA(RandomInt)
			
			if pNPCUnit:GetIsRight() == true then
				LHandPos.x = LHandPos.x - ( pNPCUnit:GetDirVector().x * RandomVector.x )
				LHandPos.y = LHandPos.y - ( pNPCUnit:GetDirVector().y * RandomVector.x )
				LHandPos.z = LHandPos.z - ( pNPCUnit:GetDirVector().z * RandomVector.x )
			else
				LHandPos.x = LHandPos.x + ( pNPCUnit:GetDirVector().x * RandomVector.x )
				LHandPos.y = LHandPos.y + ( pNPCUnit:GetDirVector().y * RandomVector.x )
				LHandPos.z = LHandPos.z + ( pNPCUnit:GetDirVector().z * RandomVector.x )
			...
			LHandPos.y = LHandPos.y + RandomVector.y
			
			local pCEffect = pDamageEffect:CreateInstance_LUA( pNPCUnit, "EVE_ENERGY_NEEDLES", LHandPos, landPos.y )
			
			pNPCUnit:SetTimerRestart( 0 )
			
			CountNiddle = CountNiddle + 1
			
			if CountNiddle == 10 then
				pNPCUnit:SetFlag_LUA(0, false)
			end
		end
	end
...
function APINK_ANGEL_05_EVE_ATTACK_C_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )
	...
	for i = 0, 9 do 
		if pNPCUnit:AnimEventTimer_LUA( 0.3 + i * 0.12 ) then		    
