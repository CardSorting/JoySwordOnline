...
function GLITER_TOMB_START_STATE_START( pKTDXApp, pX2Game, pNPCUnit )
	pNPCUnit:SetInt_LUA( 0, pX2Game:LiveNPCNumType_LUA( NPC_UNIT_ID["NUI_GLITER_ZOMBIE"] ) + 2 )
...
function CF_GLITER_TOMB_RESPAWN( pKTDXApp, pX2Game, pNPCUnit )
	local nZombie = pX2Game:LiveNPCNumType_LUA( NPC_UNIT_ID["NUI_GLITER_ZOMBIE"] )
	local nCount = 23
...
function GLITER_TOMB_RESPAWN_FRAME_MOVE( pKTDXApp, pX2Game, pNPCUnit )
	if pNPCUnit:AnimEventTimer_LUA( 0.2 ) then		
		...
		if pX2Game:IsHost() then 
			...
			if true == pNPCUnit:GetFlag_LUA( 0 ) then
				deHP = pNPCUnit:GetMaxHP() * 0.14
				pX2Game:CreateNPCReq_LUA( NPC_UNIT_ID["NUI_GLITER_ZOMBIE"], pNPCUnit:GetHardLevel(), FALSE, pos, pNPCUnit:GetIsRight(), 0, true  )
			else
				pX2Game:CreateNPCReq_LUA( NPC_UNIT_ID["NUI_GLITER_ZOMBIE"], pNPCUnit:GetHardLevel(), TRUE, pos, pNPCUnit:GetIsRight(), 0, false  )
			end
